...
public class TransactionLog {
    ...
    /**
     * Get recent transactions (all players)
     */
    public List<TransactionRecord> getRecentTransactions(int limit) {
        List<TransactionRecord> records = new ArrayList<>();
        try (Connection conn = VonixCore.getInstance().getDatabase().getConnection()) {
            PreparedStatement stmt = conn.prepareStatement(
                    "SELECT * FROM vonixcore_transactions ORDER BY timestamp DESC LIMIT ?");
            stmt.setInt(1, limit);
            ResultSet rs = stmt.executeQuery();
            ...
            VonixCore.LOGGER.warn("Failed to get recent transactions: {}", e.getMessage());
        }
        return records;
    }
