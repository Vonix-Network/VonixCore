 ...
 * Ported to Forge 1.18.2 with TextComponent refactoring.
 */
public class DiscordManager {
    ...
    public String generateLinkCode(ServerPlayer player) {
        if (linkedAccountsManager == null || !DiscordConfig.CONFIG.enabled.get()) {
            return null;
        }
    ...
    private void onJavacordConnected(String channelId) {
        try {
            ...
            if (DiscordConfig.CONFIG.enabled.get() && linkedAccountsManager != null) {
    ...
    private void processJavacordMessage(org.javacord.api.event.message.MessageCreateEvent event) {
        try {
            ...
            if (server != null) {
                ...
                if (isWebhook) {
                    finalComponent = toMinecraftComponentWithLinks(formattedMessage);
                } else {
                    ...
                    if (rawFormat.contains("[Discord]") && inviteUrl != null && !inviteUrl.isEmpty()) {
                        ...
                        if (parts.length > 0 && !parts[0].isEmpty()) {
                            builder.append(toMinecraftComponentWithLinks(parts[0]));
                        }

                        // Clickable [Discord]
                        Component discordComponent = new TextComponent("[Discord]")
                        ...
                        builder.append(discordComponent);

                        // Part after [Discord]
                        if (parts.length > 1 && !parts[1].isEmpty()) {
                            builder.append(toMinecraftComponentWithLinks(parts[1]));
                        }
                        finalComponent = builder;
                    } else {
                        finalComponent = toMinecraftComponentWithLinks(rawFormat);
                    }
                }
                ...
                server.execute(() -> {
                    for (ServerPlayer player : server.getPlayerList().getPlayers()) {
                        ...
                        // Use sendMessage with NIL_UUID for system message behavior in 1.18.2
    ...
    public void sendPlayerJoin(ServerPlayer player) {
        if (!running || !DiscordConfig.CONFIG.sendJoin.get())
            return;
        sendJoinEmbed(player.getName().getString());
    }

    public void sendPlayerLeave(ServerPlayer player) {
        if (!running || !DiscordConfig.CONFIG.sendLeave.get())
            return;
    ...
    public void updateBotStatus() {
        if (discordApi == null || !DiscordConfig.CONFIG.enabled.get()) {
            return;
        }
    ...
    private void sendWebhookEmbedToUrl(String webhookUrl, java.util.function.Consumer<JsonObject> customize) {
        ...
        VonixCore.executeAsync(() -> {
            ...
            try (Response response = httpClient.newCall(request).execute()) {
                if (!response.isSuccessful() && DiscordConfig.CONFIG.enabled.get()) {
    ...
    private Component toMinecraftComponentWithLinks(String text) {
        ...
        while (matcher.find()) {
            ...
            linkComponent.setStyle(net.minecraft.network.chat.Style.EMPTY
    ...
    private void processAdvancementEmbed(org.javacord.api.entity.message.embed.Embed embed,
        ...
        try {
            ...
            if (server != null) {
                // Use sendMessage with NIL_UUID for system message behavior in 1.18.2
                for (ServerPlayer player : server.getPlayerList().getPlayers()) {
                    player.sendMessage(advancementComponent, Util.NIL_UUID);
                }
                return;
            }
        ...
        try {
            ...
            if (playerName != null && advancementTitle != null) {
                ...
                if (server != null && fallbackComponent != null) {
                    // Use sendMessage with NIL_UUID for system message behavior in 1.18.2
                    for (ServerPlayer player : server.getPlayerList().getPlayers()) {
                        player.sendMessage(fallbackComponent, Util.NIL_UUID);
                    }
                }
            }
        } catch (Exception e) {
