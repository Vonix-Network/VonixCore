...
public class WorldCommands {
    ...
    private static int lightningAtPlayer(CommandContext<CommandSourceStack> ctx) {
        ...
        // In 1.18.2, serverLevel() is getLevel()
    ...
    private static int toggleAfk(CommandContext<CommandSourceStack> ctx, String message) {
        ...
        if (afkTime.containsKey(uuid)) {
            ...
            broadcastToAll(player.getServer(), "§7" + player.getName().getString() + " is no longer AFK");
            ...
            broadcastToAll(player.getServer(), broadcast);
            player.sendMessage(new TextComponent("§eYou are now AFK" + (message != null ? ": " + message : "")),
                    player.getUUID());
        }
        return 1;
    }
