...
/**
 * CoreProtect-style protection commands.
...
public class ProtectionCommands {
    ...
    // Pending rollback confirmations
    private static final Map<UUID, RollbackData> pendingRollbacks = new ConcurrentHashMap<>();
    ...
    private static int helpCommand(CommandContext<CommandSourceStack> ctx) {
        ...
        source.sendSuccess(() -> Component.literal("§6§l=== VonixCore Protection (CoreProtect) ==="), false);
        source.sendSuccess(() -> Component.literal("§b/co inspect §7- Toggle inspector mode"), false);
        source.sendSuccess(() -> Component.literal("§b/co lookup u:<user> t:<time> r:<radius> §7- Search logs"), false);
        source.sendSuccess(() -> Component.literal("§b/co rollback u:<user> t:<time> r:<radius> §7- Rollback changes"),
                false);
        source.sendSuccess(() -> Component.literal("§b/co restore u:<user> t:<time> r:<radius> §7- Restore changes"),
                false);
        source.sendSuccess(() -> Component.literal("§b/co undo §7- Undo last rollback/restore"), false);
        source.sendSuccess(() -> Component.literal("§b/co purge t:<time> §7- Delete old data"), false);
        source.sendSuccess(() -> Component.literal("§b/co near [radius] §7- Lookup nearby changes"), false);
        source.sendSuccess(() -> Component.literal("§b/co status §7- Show database status"), false);
        source.sendSuccess(() -> Component.literal(""), false);
        source.sendSuccess(() -> Component.literal("§7Parameters:"), false);
        source.sendSuccess(
                () -> Component.literal("§7  u:<user> §f- Player name (use #creeper, #tnt, etc. for entities)"), false);
        source.sendSuccess(() -> Component.literal("§7  t:<time> §f- Time (e.g., 1h, 3d, 1w)"), false);
        source.sendSuccess(() -> Component.literal("§7  r:<radius> §f- Radius (default: 10)"), false);
        source.sendSuccess(
                () -> Component.literal("§7  a:<action> §f- Action filter (block, container, +block, -block)"), false);
        source.sendSuccess(() -> Component.literal("§7  b:<block> §f- Block type filter (e.g., diamond_ore)"), false);
        source.sendSuccess(() -> Component.literal("§7  e:<exclude> §f- Exclude blocks/actions"), false);
    ...
    private static int inspectCommand(CommandContext<CommandSourceStack> ctx) {
        ...
        if (inspectorMode.contains(uuid)) {
            ...
            ctx.getSource().sendSuccess(() -> Component.literal("§6[VonixCore] §fInspector mode §cdisabled§f."), false);
        } else {
            inspectorMode.add(uuid);
            ctx.getSource().sendSuccess(
                    () -> Component
                            .literal("§6[VonixCore] §fInspector mode §aenabled§f. Click on blocks to see history."),
                    false);
        }
        return 1;
    }
    ...
    public static void inspectBlock(ServerPlayer player, BlockPos pos) {
        ...
        VonixCore.getInstance().getDatabase().getExecutor().execute(() -> {
    ...
    private static int executeRollbackRestore(CommandContext<CommandSourceStack> ctx, String params,
        ...
        ctx.getSource().sendSuccess(() -> Component.literal(
                String.format("§6[VonixCore] §fStarting %s... (r:%d t:%s)",
    ...
    private static int undoCommand(CommandContext<CommandSourceStack> ctx) {
        ...
        String operation = lastOp.wasRollback ? "rollback" : "restore";
        ...
        int finalModified = modified;
        ctx.getSource().sendSuccess(() -> Component.literal(String.format(
                "§6[VonixCore] §fUndo complete. §a%d §fblocks restored.", finalModified)), false);
    ...
    private static int purgeCommand(CommandContext<CommandSourceStack> ctx) {
        ...
        ctx.getSource().sendSuccess(() -> Component.literal(
                String.format("§6[VonixCore] §fPurging data older than %s...", formatDuration(seconds))), false);
        ...
        VonixCore.getInstance().getDatabase().getExecutor().execute(() -> {
            try (Connection conn = VonixCore.getInstance().getDatabase().getConnection()) {
                ...
                ctx.getSource().getServer().execute(() -> ctx.getSource().sendSuccess(() -> Component.literal(
        ...
        VonixCore.getInstance().getDatabase().getExecutor().execute(() -> {
            try (Connection conn = VonixCore.getInstance().getDatabase().getConnection()) {
                ...
                ctx.getSource().getServer().execute(() -> {
                    ctx.getSource().sendSuccess(() -> Component.literal("§6§l=== VonixCore Protection Status ==="),
                            false);
                    ctx.getSource().sendSuccess(
                            () -> Component.literal(String.format("§7Block logs: §f%,d", finalBlockCount)), false);
                    ctx.getSource().sendSuccess(
                            () -> Component.literal(String.format("§7Container logs: §f%,d", finalContainerCount)),
                            false);
                    ctx.getSource().sendSuccess(() -> Component.literal(String.format("§7Queue size: §f%d",
                            network.vonix.vonixcore.consumer.Consumer.getInstance().getQueueSize())), false);
                });
    ...
    private static BlockState getBlockState(String blockId) {
        ...
        try {
            net.minecraft.resources.ResourceLocation loc = net.minecraft.resources.ResourceLocation.parse(blockId);
            return net.minecraft.core.registries.BuiltInRegistries.BLOCK.get(loc).defaultBlockState();
        } catch (Exception e) {
            return Blocks.AIR.defaultBlockState();
        }
    }
